...
b2World::b2World(const b2Vec2 &gravity, b2ThreadPool *threadPool)
        : m_nonStaticBodies(b2_initialNonStaticBodiesCapacity), m_staticBodies(b2_initialStaticBodiesCapacity) {
...
void b2World::CollideMT() {
    ...
    group.SubmitRangedTasks(contactsTasks, m_threadCount, m_contactManager.m_contactsNonTOI.GetCount(),
                            m_stackAllocator);

    // Submit tasks for TOI contacts.
    group.SubmitRangedTasks(toiContactsTasks, m_threadCount, m_contactManager.m_contactsTOI.GetCount(),
                            m_stackAllocator);
...
void b2World::Step(float32 dt, int32 velocityIterations, int32 positionIterations) {
    ...
    // Update contacts. This is where some contacts are destroyed.
    {
        ...
        if (IsMultithreadedStepEnabled()) {
            ...
            m_contactManager.Collide(m_contactManager.m_contactsNonTOI.Data(),
                                     m_contactManager.m_contactsNonTOI.GetCount());
...
void b2World::DrawShape(b2Fixture *fixture, const b2Transform &xf, const b2Color &color) {
    ...
    auto shape = fixture->m_shape;
    if (shape) {
        auto type = shape->m_type;
        if (type) {
